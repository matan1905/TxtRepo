def parse_dsl(dsl_string: str) -> Dict[str, Any]:
    ...
    if dsl_string == "delete":
        instructions['delete'] = 'full'
    elif dsl_string.startswith("delete:"):
        _, delete_type = dsl_string.split(':')
        if delete_type in ['full', 'content']:
            instructions['delete'] = delete_type
        else:
    ...
    for file in files:
        ...
        try:
            ...
            if 'delete' in dsl:
                ...
                elif dsl['delete'] == 'content':
                    if file_path.exists():
                        with open(file_path, 'w') as f:
                            f.write('')
                        logging.info(f"Cleared content of file: {file_path}")
                    else:
                        logging.warning(f"Attempted to clear content of non-existent file: {file_path}")
