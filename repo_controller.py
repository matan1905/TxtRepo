...
def create_pull_request(repo_path: Path, github_token: str, source_branch: str, pr_branch: Optional[str], pr_title: Optional[str], pr_description: Optional[str]):
    try:
...
async def apply_changes_and_create_pr(pr_request: PullRequestRequest, background_tasks: BackgroundTasks):
    ...
    try:
        pr_url = create_pull_request(repo_path, pr_request.github_token, pr_request.branch,
                                     pr_request.pr_branch, pr_request.pr_title, pr_request.pr_description)
